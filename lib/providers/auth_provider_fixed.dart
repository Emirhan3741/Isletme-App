...
class AuthProvider with ChangeNotifier {
  // State variables ✅ String tipi kullanıyoruz
  UserModel? _user;
  String? _role;  // ✅ String olarak değiştirildi (UserRole yerine)
  ...
  UserModel? get currentUser => _user;  // ✅ Eklendi
  String? get role => _role;  // ✅ String tip
  ...
  // Role-based access control ✅ String comparison
  bool get isAdmin => _user?.role == 'admin' || _user?.role == 'owner';
  bool get isEmployee => _user?.role == 'worker' || _user?.role == 'manager';
  String get userRole => _user?.role ?? 'guest';  // ✅ String return
  
  // ✅ Eksik getter'lar eklendi
  bool get canManageFinances => isAdmin || _user?.role == 'manager';
  ...
  void setUser(UserModel? userModel) {
    if (userModel != null) {
      _user = userModel;
      _role = userModel.role;  // ✅ String to String
    } else {
      _user = null;
      _role = null;
    }
  ...
  Future<bool> signInWithEmail(String email, String password) async {
    try {
      ...
      if (credential.user != null) {
        ...
        await _sendWelcomeEmailIfNeeded(credential.user!, 'email', isNewUser: false);
        return true;
      }
      return false;
    } catch (e) {
  ...
  /// Kullanıcı state güncelleme ✅ displayName kaldırıldı
  Future<void> _updateUserState(User firebaseUser) async {
    try {
      final userDoc = await _userService.getUser(firebaseUser.uid);
      if (userDoc != null) {
        _user = userDoc;
        _role = userDoc.role;  // ✅ String to String
        notifyListeners();
      } else {
        // Fallback user model oluştur
        _user = UserModel(
          id: firebaseUser.uid,
          name: firebaseUser.displayName ?? 'Kullanıcı',
          email: firebaseUser.email ?? '',  // ✅ Email required
          role: 'worker',  // ✅ String
          sector: 'general',
        );
        _role = 'worker';
  ...
  /// FCM token kaydetme ✅ 2 parametre
  ...
  /// Hoş geldin e-postası ✅ Named parameters
  ...
  /// Alias method ✅ 3 parametre (register_page.dart uyumluluğu için)
